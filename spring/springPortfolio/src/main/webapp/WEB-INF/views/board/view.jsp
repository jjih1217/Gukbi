...
<c:choose>
<c:when test="${boardView == 'viewPasswdPage'}">
	<div class="secret">
	<h2 class="tit">비밀글입니다</h2>
		...
		<div class="passwdArea">
			<c:choose>
				<c:when test="${sessionScope.sessionNo != null && sessionScope.sessionNo == '3' && sessionScope.sessionId == 'admin'}">
					<input type="password" name="viewPasswd" value="${dto.passwd }">
				</c:when>
				<c:otherwise>
			...
			<a href="#" onClick="passwdSubmit();">확인</a>
		</div>
	</form>
	<script>
		function passwdSubmit() {
			if(confirm('전송할까요?')) {
				document.passwdForm.action = "${path}/board/view";
					...
					<div class="pic_area">
						<c:choose>
							...
							<c:otherwise>
								...
								<c:set var="temp1" value="${tempArray1[0]}"></c:set>
								<c:set var="temp2" value="${tempArray1[1]}"></c:set>
								<img src="${path }/attach${path}/member/${temp2}" alt="${tempArray1[0]}">
							</c:otherwise>
						</c:choose>
					</div>
				</div> --%>
